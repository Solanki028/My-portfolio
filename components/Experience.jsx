...
export default function Experience() {
    ...
    return (
        <section ref={ref} id="experience" className="py-32 relative overflow-hidden bg-black">
            {/* Background Elements */}
            <div className="absolute inset-0 bg-[linear-gradient(rgba(252,194,0,0.02)_1px,transparent_1px),linear-gradient(90deg,rgba(252,194,0,0.02)_1px,transparent_1px)] bg-[size:120px_120px]" />
            <div className="absolute top-0 right-0 w-1/2 h-1/2 bg-yellow-900/10 blur-3xl -z-10" />
            <div className="absolute bottom-0 left-0 w-1/2 h-1/2 bg-yellow-900/10 blur-3xl -z-10" />
                    ...
                    <div className="relative space-y-8">
                        ...
                        {experiences.map((exp, index) => (
                            ...
                            >
                                {/* Timeline Dot */}
                                <div
                                    className={`absolute left-4 md:left-1/2 w-4 h-4 rounded-full bg-black border-2 border-yellow-500 transform -translate-x-1/2 z-10 box-content transition-all duration-500 delay-300 ${isVisible ? 'scale-100 opacity-100' : 'scale-0 opacity-0'
                                        }`}
                                ...
                                <div className={`w-full md:w-1/2 pl-12 md:pl-0 ${index % 2 === 0 ? 'md:pl-16' : 'md:pr-16'
                                    ...
                                    <div
                                        className={`group relative bg-zinc-900/40 backdrop-blur-xl border border-white/5 rounded-2xl p-6 hover:border-yellow-500/30 transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl hover:shadow-yellow-500/10 ${isVisible ? (index % 2 === 0 ? 'animate-slide-left' : 'animate-slide-right') : 'opacity-0'
                                            }`}
                                    ...
                                    >
                                        ...
                                        {/* Hover Decoration */}
                                        <div className={`absolute inset-0 ${exp.color} opacity-0 group-hover:opacity-5 rounded-2xl transition-opacity duration-300 -z-10`} />
                                        <div className="absolute -right-1 -bottom-1 w-10 h-10 bg-yellow-500/20 blur-xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                                    </div>
                                </div>
